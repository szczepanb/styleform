$.widget( "Imperial.styleForm", {
    ...
    _create: function() {
		...
		if($this.options.checkbox.use)
			$this.element.find('input[type="checkbox"]').checkboxStyle($this.options.checkbox);
		if($this.options.radio.use)
			$this.element.find('input[type="radio"]').radioStyle($this.options.radio);
		
		if($this.options.select.use)
		{	
			$this.element.find('select').each(function()
			{
				$(this).selectStyle($this.options.select);
				if(!$(this).hasClass('i-s-styled'))
					$(this).removeData();
			});
		}
		
		if($this.options.fileUploader.use)
			$this.element.find('input[type="file"]').fileUploaderStyle($this.options.fileUploader);
		
		//Odejść od tego zostawić tylko na intervalu bo jest lepsze
//		$(document).ajaxSuccess(function( event, xhr, settings ) {
//			if($this.options.checkbox.use)
//				$this.element.find('input[type="checkbox"]').not('.i-s-styled').checkboxStyle($this.options.checkbox);
//			if($this.options.radio.use)
//				$this.element.find('input[type="radio"]').not('.i-s-styled').radioStyle($this.options.radio);
//			if($this.options.select.use)
//				$this.element.find('select').not('.i-s-styled').selectStyle($this.options.select);
//			if($this.options.fileUploader.use)
//				$this.element.find('input[type="file"]').not('.i-s-styled').fileUploaderStyle($this.options.fileUploader);
//		});
		...
		$(document).off('domChange.'+$this.uuid).on('domChange.'+$this.uuid, function()
		{
			...
			if($this.options.checkbox.use)
				$this.element.find('input[type="checkbox"]').not('.i-ch-styled').checkboxStyle($this.options.checkbox);
			if($this.options.radio.use)
				$this.element.find('input[type="radio"]').not('.i-r-styled').radioStyle($this.options.radio);
			if($this.options.select.use)
			{	
				$this.element.find('select').not('.i-s-styled').selectStyle($this.options.select);
			}
...
//				});
	...
	_destroy:function()
	{
		...
		this.element.closest('.i-s-wrapper').replaceWith(this.element);
	},
	destroy:function(){this._destroy();},
		...
		if(this.options.absolute)
		{
			...
			$(document).off('selectHide').on('selectHide', function()
			{
				$list.css({
					...
					top: ($this.element.closest('.i-s-wrapper').offset().top+$this.element.closest('.i-s-wrapper').outerHeight())+"px"
				});
			});
	...
	_open:function(){
		...
		if($this.options.absolute)
		{
			$("#i-s-list-"+$this.uuid).find('.i-s-list').removeClass('i-hide-opacity').attr('tabindex', '1').focus();
		}
		else 
		{
			...
			if($(this).is('option'))
			{
				...
				if($this.options.absolute)
				{
					if(!($this.element.data().active_not_show_on_list && $(this).is(':selected')))
						$('#i-s-list-'+$this.uuid).find('ul.i-s-list-inner').append('<li class="i-s-default'+is_selected+is_disabled+'"><input type="hidden" disabled value="'+$(this).val()+'" />'+html+'</li>');
				}
				else {
					if(!($this.element.data().active_not_show_on_list && $(this).is(':selected')))
						$this.element.closest('.i-s-wrapper').find('ul.i-s-list-inner').append('<li class="i-s-default'+is_selected+is_disabled+'"><input type="hidden" disabled value="'+$(this).val()+'" />'+html+'</li>');
				}
			}
			else
			{
				...
				$(this).find('option').each(function()
				{
					...
					if(!($this.element.data().active_not_show_on_list && $(this).is(':selected')))
						$wrap.find('ul').append('<li class="i-s-default'+is_selected+is_disabled+'"><input type="hidden" disabled value="'+$(this).val()+'" />'+html+'</li>');
				});
...
//	widgetEventPrefix: "imperial",
	...
	_destroy:function()
	{
		this.element.removeClass('i-ch-styled');
